...
async def show_schedule_options(callback: types.CallbackQuery, callback_data: MyCallback):
    faculty, course, group = callback_data.val.split('//')[0], callback_data.val.split('//')[1], callback_data.val.split('//')[2]
...
async def ch_rasp_date(callback: types.CallbackQuery, callback_data: MyCallback):
    ...
    if callback_data.val == 'База расписания':
        ...
        # Определяем, нужно ли делить сообщение на части
        if len(full_message) > 4096:
            # Находим количество частей, на которые необходимо разделить сообщение
            num_parts = len(full_message) // 4096 + 1
            part_size = len(user_parts) // num_parts
            
            # Делим список пользователей на соответствующее количество частей
            parts = []
            for i in range(0, len(user_parts), part_size):
                part = user_parts[i:i + part_size]
                message_part = '\n'.join(part)
                # Добавляем заголовок только к первой части
                if i == 0:
                    message_part = f'Пользователей в базе - {len(data_user)} человек\n\n' + message_part
